...
public class MyGraph<E, F extends LinkedList<E>> {
    ...
    public void sortRelations() {
        HashMap<Character, LinkedList<Character>> newGraph = new HashMap<>();
        for (Object key : graph.keySet()) {
            List<Character> data = (List<Character>) graph.get(key);
            Collections.sort(data);
            newGraph.put((Character) key, (LinkedList<Character>) data);
        }

        graph = (HashMap<E, F>) newGraph;
    }
